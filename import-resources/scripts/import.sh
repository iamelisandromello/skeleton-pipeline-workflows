  ...
  # ✅ Exporta TF_VAR_lambda_layer_arn se fornecido
  if [ -n "$LAMBDA_LAYER_ARN" ]; then
    export TF_VAR_lambda_layer_arn="$LAMBDA_LAYER_ARN"
    echo "✅ LAMBDA_LAYER_ARN fornecido e exportado como TF_VAR_lambda_layer_arn"
  else
    echo "ℹ️ LAMBDA_LAYER_ARN não definido. Pulando importação da Lambda Layer."
  fi
  ...
  if [ -n "$LAMBDA_LAYER_ARN" ]; then
    ...
    echo "ℹ️ LAMBDA_LAYER_ARN não definido. Nenhuma Layer será associada."
  fi
